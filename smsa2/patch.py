...
SMSA2_PLAYER_NAME_BYTE_LENGTH = 64
...
def update_dol_offsets(gcm: GCM, dol: DOL, seed: str, slot_name: str, starting_nozzle: int, level_access: bool,
    ...
    bin_data.seek(0x04)
    bin_data.write(bytes.fromhex("4132"))
    bin_data.seek(0x34)
    bin_data.write(bytes.fromhex("534D5320417263616465203220417263686970656C61676F"))
    bin_data.seek(0x570514E8)
    bin_data.write(bytes.fromhex("534D5320417263616465203220417263686970656C61676F"))
    bin_data.seek(0x57051528)
    bin_data.write(bytes.fromhex("534D5320417263616465203220417263686970656C61676F"))
    ...
    custom_dol_code = read_binary(__name__, "SMSA2_custom_code.smsco")
    ...
    dol.data.write(sbf.string_to_bytes(slot_name, SMSA2_PLAYER_NAME_BYTE_LENGTH))
