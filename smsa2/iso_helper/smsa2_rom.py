...
MAIN_PKG_NAME = "worlds.supermariosunshinearcade2.SMSA2Patcher"

RANDOMIZER_NAME = "Super Mario Sunshine Arcade 2"
SMSA2_USA_MD5 = 0xB01AD744009EEEE5E0171ACC84FF42E5

class InvalidCleanISOError(Exception):
    """
    Exception raised for when user has an issue with their provided Super Mario Sunshine Arcade 2 ISO.
...
class SMSA2PlayerContainer(APPlayerContainer):
    game = RANDOMIZER_NAME
    compression_method = zipfile.ZIP_DEFLATED
    patch_file_ending = ".apsmsa2"
    ...
    def write_contents(self, opened_zipfile: zipfile.ZipFile) -> None:
        opened_zipfile.writestr("patch.apsmsa2", json.dumps(self.output_data, indent=4, default=convert_to_base_types))
        super().write_contents(opened_zipfile)

class SMSA2Patch(APPatch, metaclass=AutoPatchRegister):
    game = RANDOMIZER_NAME
    hash = SMSA2_USA_MD5
    patch_file_ending = ".apsmsa2"
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        super(SMSA2Patch, self).__init__(*args, **kwargs)
    ...
    def __get_temp_folder_name(self) -> str:
        ...
        # from ..SMSA2Client import CLIENT_VERSION
        temp_path = os.path.join(tempfile.gettempdir(), "super_mario_sunshine_arcade_2", CLIENT_VERSION, "libs")
        return temp_path

    def patch(self, apsmsa2_patch: str) -> str:
        # Get the AP Path for the base ROM
        smsa2_clean_iso = self.get_base_rom_path()
        logger.info("Provided Super Mario Sunshine Arcade 2 ISO Path was: " + smsa2_clean_iso)

        base_path = os.path.splitext(apsmsa2_patch)[0]
        output_file = base_path + self.result_file_ending

        try:
            # Verify we have a clean rom of the game first
            self.verify_base_rom(smsa2_clean_iso, throw_on_missing_speedups=True)

            # Use our randomize function to patch the file into an ISO.
            from ..SMSA2Patcher import SuperMarioSunshineArcade2Randomizer
            with zipfile.ZipFile(apsmsa2_patch, "r") as zf:
                apsmsa2_bytes = zf.read("patch.apsmsa2")
            SuperMarioSunshineArcade2Randomizer(smsa2_clean_iso, output_file, apsmsa2_bytes)
        except ImportError:
            self.__get_remote_dependencies_and_create_iso(apsmsa2_patch, output_file, smsa2_clean_iso)
        return output_file

    def read_contents(self, apsmsa2_patch: str) -> dict[str, Any]:
        with zipfile.ZipFile(apsmsa2_patch, "r") as zf:
    ...
    def get_base_rom_path(cls) -> str:
        ...
        file_name = options["smsa2_options"]["iso_file"]
        if not os.path.exists(file_name):
            file_name = Utils.user_path(file_name)
        return file_name

    @classmethod
    def verify_base_rom(cls, smsa2_rom_path: str, throw_on_missing_speedups: bool = False):
        # Verifies we have a valid installation of Super Mario Sunshine USA. There are some regional file differences.
        logger.info("Verifying if the provided ISO is a valid copy of Super Mario Sunshine Arcade 2 USA edition.")
        ...
        if yaz0_yay0.PY_FAST_YAZ0_YAY0_INSTALLED:
            ...
            if throw_on_missing_speedups:
                ...
                raise ImportError("Cannot continue patching Super Mario Sunshine Arcade 2 due to missing libraries.")
            logger.info("Continuing patching without speedups.")

        base_md5 = md5()
        with open(smsa2_rom_path, "rb") as f:
    ...
    def download_lib_zip(self, tmp_dir_path: str) -> None:
        logger.info("Getting missing dependencies for Super Mario Sunshine Arcade 2 from remote source.")
        ...
        try:
            ...
            logger.error("While trying to download SMSA2 dependencies from the release page, an unexpected error " +
    ...
    def create_iso(self, temp_dir_path: str, patch_file_path: str, output_iso_path: str, vanilla_iso_path: str):
        ...
        with zipfile.ZipFile(patch_file_path, "r") as zf:
            apsmsa2_bytes = zf.read("patch.apsmsa2")
        SuperMarioSunshineArcade2Randomizer(vanilla_iso_path, output_iso_path, apsmsa2_bytes)

    def __get_remote_dependencies_and_create_iso(self, apsmsa2_patch: str, output_file: str, smsa2_clean_iso: str):
        try:
            ...
            self.create_iso(local_dir_path, apsmsa2_patch, output_file, smsa2_clean_iso)
